...
async def add_to_watchlist(
    ...
    cursor = conn.cursor()
    cursor.execute(
        """INSERT INTO watchlist (owner_key, title, type, genre, watched, added_at)
           VALUES (?, ?, ?, ?, 0, ?)""",
...
async def mark_watched(
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM watchlist WHERE id = ? AND owner_key = ?",
        (item_id, x_api_key)
    )
    ...
    cursor.execute(
        "UPDATE watchlist SET watched = 1 WHERE id = ?",
        (item_id,)
    )
    conn.commit()
...
async def rate_item(
    ...
    cursor.execute(
        "UPDATE watchlist SET rating = ? , review = ? WHERE id = ?",
        (body.rating, body.review, item_id)
    )
    conn.commit()
    cursor.execute("select * FROM watchlist WHERE id = ?", (item_id,))
    updated = cursor.fetchone()
...
# ts taking too longerrr than i thoughtttt
#especially copying pasting the almost same code snippet with so minimum changes

@app.delete("/watchlist/{item_id}", tags = ["Watchlist"], dependencies=[Depends(verify_api_key)])
@Limiter.limit("67/minute")    #6-7 xd
...
async def get_stats(
    ...
    cursor.execute(
        "SELECT AVG(rating) FROM watchlist WHERE owner_key = ? AND rating IS NOT NULL",
        (x_api_key,)
    )
    avg_rating = cursor.fetchone()[0]

    cursor.execute(
        """SELECT genre, COUNT(*) as count FROM watchlist
        WHERE owner_key = ? GROUP BY genre ORDER BY count DESC LIMIT 1""",
        (x_api_key, )
    )
...
def main():
    import uvicorn
    uvicorn.run("resolution_week4_YOUR_USERNAME.main:app", host="127.0.0.1", port=8000)
